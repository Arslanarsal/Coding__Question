...
long getAns(long *Arr, int ind, int buy, int n, vector<vector<long>> &dp)
{
    ...
    { // We can buy the stock
        profit = getAns(Arr, ind + 1, 0, n, dp);
    }
    ...
    { // We can sell the stock
        profit = max(getAns(Arr, ind + 1, 1, n, dp), Arr[ind] + getAns(Arr, ind + 1, 2, n, dp));
    }
...
class Solution
{
    long getAns(int ind, int buy, vector<int> &Arr, int n, vector<vector<long>> &dp)
    {
        ...
        if (buy == 2)
        { // We can buy the stock
            profit = getAns(ind + 1, 0, Arr, n, dp);
        }
        ...
        if (buy == 1)
        { // We can sell the stock
            profit = max(getAns(ind + 1, 1, Arr, n, dp), Arr[ind] + getAns(ind + 1, 2, Arr, n, dp));
        }
    ...
    int maxProfit(vector<int> &prices)
    {
        ...
        for (int ind = n - 1; ind >= 0; ind--)
        {
            for (int buy = 0; buy < 3; buy++)
            {
                ...
                if (buy == 0)
                { // We can buy the stock
                    profit = max(next[0], -prices[ind] + next[1]);
                }

                if (buy == 1)
                { // We can sell the stock
                    profit = max(next[1], prices[ind] + next[2]);
                }
