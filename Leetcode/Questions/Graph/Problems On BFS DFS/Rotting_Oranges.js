...
var orangesRotting = function (grid) {
    ...
    let visit = Array.from({ length: n }, () => new Array(m).fill(0));
    ...
    while (queu.length) {
        let it = queu.shift();
        ans = Math.max(ans, it[2]);
        for (let i = 0; i < 4; i++) {
            let newi = it[0] + x[i];
            let newy = it[1] + y[i];
            if (newi >= 0 && newi < n && newj >= 0 && newj < m && !visit[newi][newj] && grid[newi][newj] === 1) {
                visit[newi][newj] = 1; 
                queu.push([newi, newj, it[2] + 1]);
            }
        }
    }
    for (let i = 0; i < n; i++) {
